...
const Navbar = () => {
  ...
  return (
    <>
      ...
      >
        <div className="container mx-auto px-6 relative">
          <div className="flex items-center justify-between">
            {/* Logo */}
            <a href="/" onClick={(e) => { e.preventDefault(); handleBackToHome(); }} className="flex items-center gap-3">
              <img src={logo} alt="Artifex Logo" className="w-10 h-10 rounded-xl object-cover shadow-lg shadow-primary/20" />
              <span className={`font-bold text-xl tracking-tight transition-colors ${scrolled ? "text-surface-dark-foreground" : "text-white"}`}>
                Artifex
              </span>
            </a>
                      ...
                      {navLinks.map((link) => (
                        <a
                          ...
                          onClick={(e) => {
                            ...
                            if (link.name !== "About" && link.name !== "Works") setIsOpen(false);
                          }}
      ...
      <WorksModal isOpen={isWorksOpen} onClose={() => setIsWorksOpen(false)} />
    </>
  );
};
